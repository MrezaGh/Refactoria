...
//        }
    ...
    public void sub() {
        operation(Operation.SUB, "sub");
    }

    public void mult() {
        operation(Operation.MULT, "mult");
    }

    private void operation(Operation op, String type) {
        Address temp = new Address(memory.getTemp(), VarType.Int);
        Address s2 = ss.pop();
        Address s1 = ss.pop();

        if (s1.varType != VarType.Int || s2.varType != VarType.Int) {
            ErrorHandler.printError("In " + type+ " two operands must be integer");
        }
